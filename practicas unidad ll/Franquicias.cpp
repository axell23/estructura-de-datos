//Axell Balmore Flores Henriquez FH100217  
#include <iostream>
#include <array>
#include <cstdlib>
#include <ctime>
...
char pedirLetra(char respuesta[]){
	char letra;
	cout << "Ingrese una letra o la respuesta." << endl;
	cin >> letra;
	int i=0;
	while(i=0){
...
int elegirOpcion(){
	...
	cout << "2- Buscar franquicia favorita" << endl;
	cout << "3- salir" << endl;
	cout << "Su selección";
	cin >> opcion;
	...
	while (cin.fail() || !(opcion >= 1 && opcion <= 3)){
		cin.clear();
		cin.ignore(1000, '\n');
		cout << "Opcion no valida. Su selección";
		cin >> opcion;
	}
	cout << endl;
	return opcion;
...
int busquedaBinaria(array<string, ELEMENTOS> &arr, string valor){
	...
	int indice=-1;
	 while (inf <= sup){
	 	med = inf +(sup-inf)/2;
	 	if(valor.at(0) < arr[med].at(0) && valor == arr[med]){
	 		sup = med-1;
		 }
